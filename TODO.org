...
* Further work and ideas
  ...
  - [ ] put together a list of things necessary for a release on public repos, also the
    ...
    - both function definition styles

    - go through, say, 5 top mod codebase to check compatibility, extract features
      necessary for proper work

      - FTEQCC dialect support?

    - unified local definition cache (xref, eldoc, capf, imenu)

    - definition search in open current project buffers
